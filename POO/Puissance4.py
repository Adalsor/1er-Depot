...
class Joueur:
    ...
    def ia_jouer(self):
        print("Le Joueur Rouge Joue son pion :")
        for i in range(6):

            for l in range(7):
                if l <= 3:
                    if all(board[i][l + k] == "Rouge" for k in range(3)):
                        if board[i][l+3] == "     ":
                            return self.placer(l+3)
                if i <= 2:
                    if all(board[i + k][l] == "Rouge" for k in range(3)):
                        if board[i +3][l] == "     ":
                            return self.placer(l)
                if i <= 2 and l <= 3:
                    if all(board[i + k][l + k] == "Rouge" for k in range(3)):
                        if board[i+3][l+3] == "     ":
                            return self.placer(l+3)
                if i >= 3 and l <= 3:
                    if all(board[i - k][l + k] == "Rouge" for k in range(3)):
                        if board[i-3][l + 3] == "     ":
                            return self.placer(l+3)
                if i == 5 and l == 6:
                    ia_choice = random.choice(['0', '1', '2', '3', '4', '5', '6'])
                    return self.placer(ia_choice)
